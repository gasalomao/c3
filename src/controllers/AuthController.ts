...
class AuthController {
    ...
    async signIn(req: Request, res: Response) {
        ...
        if (!body.email || !body.password) {
            return res.status(401).json({status: 401, error: 'Está faltando parâmetros' });
        }
        ...
        try {
            ...
            if (!user) {
                return res.status(401).json({status: 401, error: 'Usuário não foi localizado' });
            }

            const isPasswordValid = await validateHash(body.password, user.password);
            
            if (!isPasswordValid) {
                return res.status(401).json({status: 401, error: 'Senha errada' });
            }
            if (!jwttoken) {
